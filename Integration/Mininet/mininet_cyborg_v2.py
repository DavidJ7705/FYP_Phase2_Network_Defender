...
class CybORGTopoV2(Topo):
    """CybORG topology v2 - 2 Zones, 5 Hosts and 1 router"""
    def __init__(self):
        # Initialize topology
        Topo.__init__(self)
        ...
        # ========== ROUTER ==========
        router = self.addHost('rtr', ip='10.0.110.1/24')
        
        # ========== HOSTS (representing devices) ==========
        # admin zone (10.0.110.0/24)
        admin_server = self.addHost('asrv', ip='10.0.110.10/24', defaultRoute='via 10.0.110.1')
        admin_user = self.addHost('ausr', ip='10.0.110.20/24', defaultRoute='via 10.0.110.1')

        # operational zone (10.0.198.0/24)
        operational_server = self.addHost('osrv', ip='10.0.198.10/24', defaultRoute='via 10.0.198.1')
        operational_user_1 = self.addHost('ousr1', ip='10.0.198.20/24', defaultRoute='via 10.0.198.1')
        operational_user_2 = self.addHost('ousr2', ip='10.0.198.30/24', defaultRoute='via 10.0.198.1')

        # ========== LINKS (representing connections) ==========
        # admin zone links
        self.addLink(admin_switch, router)
        self.addLink(admin_switch, admin_server)
        self.addLink(admin_switch, admin_user)

        # operational zone links
        self.addLink(operational_switch, router)
